import { FaGithub, FaTwitter, FaLinkedin, FaEnvelope, FaMapMarkerAlt, FaHeart } from 'react-icons/fa';

const Footer = () => {
  const socialLinks = [
    { name: 'GitHub', icon: FaGithub, url: 'https://github.com/bharatfoss' },
    { name: 'Twitter', icon: FaTwitter, url: 'https://twitter.com/bharatfoss' },
    { name: 'LinkedIn', icon: FaLinkedin, url: 'https://linkedin.com/company/bharatfoss' }
  ];
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          ...
          <div className="lg:col-span-2">
            <div className="flex items-center space-x-3 mb-6">
              ...
              <div>
                <span className="text-2xl font-bold text-slate-900">BharatFOSS</span>
                <div className="text-sm text-slate-500 -mt-1">Foundation</div>
              </div>
            </div>
              ...
              {socialLinks.map((link) => {
                ...
                return (
                  <a
                    key={link.name}
                    href={link.url}
                    className="group p-3 bg-white border border-slate-200 rounded-xl shadow-sm hover:shadow-lg hover:border-blue-300 transition-all duration-300"
                    target="_blank"
                    rel="noopener noreferrer"
                  >
                    <Icon className="text-slate-600 group-hover:text-blue-600 transition-colors duration-300" size={20} />
                  </a>
                );
              })}
            </div>
            ...
            <ul className="space-y-4">
              {quickLinks.map((link) => (
                <li key={link.name}>
                  ...
                  >
                    <span className="w-0 group-hover:w-2 h-0.5 bg-blue-600 mr-0 group-hover:mr-3 transition-all duration-300"></span>
                    {link.name}
                  </a>
                </li>
              ))}
          ...
          {/* Resources */}
          <div>
            <h3 className="text-slate-900 font-bold mb-6 text-lg">Resources</h3>
            <ul className="space-y-4">
              {resources.map((link) => (
                <li key={link.name}>
                  <a 
        ...
        <div className="border-t border-slate-200 mt-12 pt-8">
          <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
            ...
            <div className="flex space-x-6 text-sm">
              ...
              <a href="#license" className="text-slate-500 hover:text-blue-600 transition-colors">Open Source License</a>
            </div>
          </div>
        </div>
      </div>
